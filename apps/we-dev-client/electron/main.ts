...
function createWindow() {
  ...
  try {
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
      ...
      const APP_BASE_URL = "https://idem appgen.ai";
      ...
      try {
        ...
        async function getAllFiles(dir: string): Promise<string[]> {
          ...
          for (const entry of entries) {
            ...
            if (
              isHiddenNodeModules.some(
                (item) => entry?.name?.indexOf(item) > -1
              )
            ) {
              continue;
            }
    ...
    ipcMain.handle(
      ...
      (event: IpcMainInvokeEvent, options: TerminalOptions): ProcessResult => {
        ...
        const processId =
          options.processId || Math.random().toString(36).substr(2, 9);
    ...
    ipcMain.on("mcp:servers-from-renderer", (_, servers) =>
      mcpService.setServers(servers)
    );
    ipcMain.handle("mcp:list-servers", async () =>
      mcpService.listAvailableServices()
    );
    ...
    ipcMain.handle("mcp:update-server", async (_, server: MCPServer) =>
      mcpService.updateServer(server)
    );
    ipcMain.handle("mcp:delete-server", async (_, serverName: string) =>
      mcpService.deleteServer(serverName)
    );
    ...
    ipcMain.handle("mcp:list-tools", async (_, serverName?: string) =>
      mcpService.listTools(serverName)
    );
    ipcMain.handle(
      "mcp:call-tool",
      async (_, params: { client: string; name: string; args: any }) =>
        mcpService.callTool(params)
    );
    ipcMain.handle("mcp:cleanup", async () => mcpService.cleanup());

    ipcMain.handle("app:is-binary-exist", (_, name: string) =>
      isBinaryExists(name)
    );
    ipcMain.handle("app:get-binary-path", (_, name: string) =>
      getBinaryPath(name)
    );
    ipcMain.handle("app:install-uv-binary", () =>
      runInstallScript("install-uv.js")
    );
    ipcMain.handle("app:install-bun-binary", () =>
